...
# %% The proposed ATCNet model, 22/14 channel mSEM-4
...
def Conv_block_(input_layer, F1=4, kernLength=64, poolSize=8, D=2, in_chans=22,
    ...
    #         if i == 0:
    #             input_m = block1[:,:,i:(i+1),:]
    #         else:
    #             va = block1[:,:,i:(i+1),:]
    #             input_m = Concatenate(axis=-2)([input_m, va])

    #     for i in [1,2,6,7,8,13,14,18]:
    #         if i == 1:
    #             input_l = block1[:,:,i:(i+1),:]
    #         else:
    #             va = block1[:,:,i:(i+1),:]
    #             input_l = Concatenate(axis=-2)([input_l, va])

    #     for i in [4,5,10,11,12,16,17,20]:
    #         if i == 4:
    #             input_r = block1[:,:,i:(i+1),:]
    #         else:
    #             va = block1[:,:,i:(i+1),:]
    #             input_r = Concatenate(axis=-2)([input_r, va])

    #     block_m = DepthwiseConv2D((1, 6), depth_multiplier=1, data_format='channels_last',
    #                              # depthwise_regularizer=L2(weightDecay),
    #                              depthwise_constraint=max_norm(maxNorm, axis=[0, 1, 2]), use_bias=False)(input_m)
    #     block_m = BatchNormalization(axis=-1)(block_m)
    #     block_l = DepthwiseConv2D((1, 8), depth_multiplier=1, data_format='channels_last',
    #                                # depthwise_regularizer=L2(weightDecay),
    #                                depthwise_constraint=max_norm(maxNorm, axis=[0, 1, 2]), use_bias=False)(input_l)
    #     block_l = BatchNormalization(axis=-1)(block_l)
    #     block_r = DepthwiseConv2D((1, 8), depth_multiplier=1, data_format='channels_last',
    #                                # depthwise_regularizer=L2(weightDecay),
    #                                depthwise_constraint=max_norm(maxNorm, axis=[0, 1, 2]), use_bias=False)(input_r)
    #     block_r = BatchNormalization(axis=-1)(block_r)
    ...
    block_m = DepthwiseConv2D((1, 4), depth_multiplier=1, data_format='channels_last',
                              # depthwise_regularizer=L2(weightDecay),
                              depthwise_constraint=max_norm(maxNorm, axis=[0, 1, 2]), use_bias=False)(input_m)
    block_m = BatchNormalization(axis=-1)(block_m)
    block_l = DepthwiseConv2D((1, 5), depth_multiplier=1, data_format='channels_last',
                              # depthwise_regularizer=L2(weightDecay),
                              depthwise_constraint=max_norm(maxNorm, axis=[0, 1, 2]), use_bias=False)(input_l)
    block_l = BatchNormalization(axis=-1)(block_l)
    block_r = DepthwiseConv2D((1, 5), depth_multiplier=1, data_format='channels_last',
                              # depthwise_regularizer=L2(weightDecay),
                              depthwise_constraint=max_norm(maxNorm, axis=[0, 1, 2]), use_bias=False)(input_r)
    block_r = BatchNormalization(axis=-1)(block_r)
    block = [block_l, block_m, block_r]
    for b in block:
        # b = Reshape((b.shape[1], 1, b.shape[2]))(b)
        block1 = Concatenate(axis=-2)([block1, b])

    block2 = DepthwiseConv2D((1, in_chans + 3), depth_multiplier=D, data_format='channels_last',
                             # depthwise_regularizer=L2(weightDecay),
